...
var module_path = path.join(__dirname, '../bin', process.platform+ '-'+ process.arch+ '-'+ v8, 'node_sqlite3');
try {
	fs.statSync(module_path + '.node');
} catch (ex) {
	// No binary!
	throw new Error('`'+ module_path + '.node` is missing. Try reinstalling `node-sqlite3`?');
}

var sqlite3 = module.exports = exports = require(module_path);
