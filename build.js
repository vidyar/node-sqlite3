...
var module_name = 'node_sqlite3.node';
...
var installPath = path.join(__dirname, 'bin', modPath, module_name);
if (!force) {
	try {
...
function afterBuild() {
	var targetPath = path.join(__dirname, 'build', module_name);
	...
	try {
		...
		console.error('Build succeeded but target not found at ' + targetPath);
		process.exit(1);
	}
	fs.rename(targetPath, installPath);
